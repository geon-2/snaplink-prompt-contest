  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        console.log('[poll] 팀 목록:', teamItems.map((t) => `${t.team_name}(${t.api_key_preview}) → ${t.status}`));
        ...
        if (
          ...
          console.log('[poll] generating→completed 감지, 상세 재로드:', selectedTeamIdRef.current);
        ...
        console.log('[poll] 등록된 팀:', teamRegistrationsRef.current.map((r) => `${r.teamName}(${r.apiKey?.slice(-4)})`));
        console.log('[poll] 이미 처리된 key(suffix):', [...autoGeneratedRef.current].map((k) => k.slice(-4)));
        ...
        for (const team of teamItems) {
          if (team.status !== 'submitted') {
            ...
            if (matched) {
              console.log(`[poll] 이미 처리됨으로 마킹: ${team.team_name}(${team.api_key_preview}) status=${team.status}`);
              autoGeneratedRef.current.add(matched.apiKey?.trim() ?? '');
            }
          }
        }
        ...
        for (const reg of teamRegistrationsRef.current) {
          ...
          if (!apiKey) { console.log(`[poll] skip: apiKey 없음 (teamName=${reg.teamName})`); continue; }
          if (autoGeneratedRef.current.has(apiKey)) { console.log(`[poll] skip: 이미 처리됨 (${reg.teamName})`); continue; }

          const match = teamItems.find(
            (team) => team.status === 'submitted' && apiKeyMatchesPreview(apiKey, team.api_key_preview),
          );
          if (!match) { console.log(`[poll] skip: 매칭 팀 없음 또는 미제출 (${reg.teamName}, suffix=${apiKey.slice(-4)})`); continue; }

          console.log(`[poll] 🚀 generate 요청: ${reg.teamName}(${match.team_name}, ${match.api_key_preview})`);
          autoGeneratedRef.current.add(apiKey);
          try {
            const submission = await generateContestSubmissionImages(trimmedKey, apiKey);
            console.log(`[poll] ✅ generate 완료: ${reg.teamName}`);
            if (cancelled) break;
            ...
            console.error(`[poll] ❌ generate 실패: ${reg.teamName}`, err);
          }
        }
      } catch (err) {
        console.error('[poll] 팀 목록 로드 실패', err);
      }
    };
